...
RUN apt-get update && apt-get install -y     build-essential     libpq-dev     python3-dev     postgresql-client     libpq5     ffmpeg     curl     && rm -rf /var/lib/apt/lists/*
...
RUN adduser --disabled-password --gecos '' --uid 1000 altoppe &&     chown -R altoppe:altoppe /app
